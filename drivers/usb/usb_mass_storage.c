...
/* Private data stored in blockdev->private so read/write callbacks know
 * which drive and LUN they belong to. */
typedef struct {
    usb_storage_t *drive;
    int            lun;
} usb_bdev_priv_t;
...
{
    usb_bdev_priv_t *p = bdev->private;
    return usb_bot_rw(p->drive, p->lun, lba, count, buf, 0);
}

ssize_t usb_block_write(blockdev_t *bdev, uint64_t lba, uint32_t count, const void *buf)
{
    usb_bdev_priv_t *p = bdev->private;
    return usb_bot_rw(p->drive, p->lun, lba, count, (void*)buf, 1);
}
