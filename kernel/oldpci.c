...
#define PCIE_ECAM_BASE  0x600000000ULL  // 6GB address space!
...
uint32_t pci_read_config(pci_dev_t *dev, uint32_t offset)
{
    ...
    return readl((void *)(PCIE_EXT_CFG_DATA + (offset & 0x3)));
}
...
void pci_write_config(pci_dev_t *dev, uint32_t offset, uint32_t value)
{
    ...
    writel(idx, (void *)PCIE_EXT_CFG_INDEX);
    writel(value, (void *)(PCIE_EXT_CFG_DATA + (offset & 0x3)));
}
...
if (vendor == 0 || vendor == 0xFFFFFFFF) {
    uart_puts("[PCI] No device - vendor = 0x00000000 or 0xFFFFFFFF\n");
    return;
}

uart_puts("[PCI] DEVICE FOUND! Non-zero vendor ID!\n");
